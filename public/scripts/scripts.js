    ...
    // сегодня (дата месяц)
    var currentHour = NumberProcessing(getMoscowDate().getHours());
    ...
    $(function () {
        ...
        if (currentHour < 7 & currentHour >= 0) {
            $('.input__date')
                .attr("min", year + "-" + monthYesterday + "-" + dayYesterday + "T07:00")
                .attr("max", year + "-" + currentMonth + "-" + currentDay + "T23:59");
        }
    ...
    function ChangeData(element) {
        ...
        if (currentHour < 7 & currentHour >= 0) {
            //соответствие указанной даты 1му дню
            CheckDays(element, dayYesterday, dataTime, day1List, 0);

            //соответствие указанной даты 2му дню
            CheckDays(element, currentDay, dataTime, day2List, 17);
        }
